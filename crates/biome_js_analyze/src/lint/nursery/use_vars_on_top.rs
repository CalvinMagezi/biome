...
impl Rule for UseVarsOnTop {
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        if !declaration.is_var() || is_var_on_top(declaration) {
            return None;
        }
...
fn is_var_on_top(declaration: &JsVariableDeclaration) -> bool {
    let Some(parent) = declaration.syntax().parent() else {
        return false;
    };

    if let Some(statement) = JsVariableStatement::cast(parent.clone()) {
        return is_top_level_var_statement(&statement).unwrap_or(false);
    }

    if let Some(clause) = JsVariableDeclarationClause::cast(parent.clone()) {
        let Some(parent) = clause.syntax().parent() else {
            return false;
        };
        ...
        if let Some(export) = JsExport::cast(parent) {
            return is_top_level_var_export(&export).unwrap_or(false);
        }
    }
